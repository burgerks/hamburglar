...
Folder layout expected next to this file:
    stimuli/shapes/         square.png, circle.png, triangle.png (bandit symbols)
...
SYMBOL_NAMES = ['square', 'circle', 'triangle']   # logical symbol order; files in stimuli/shapes/
...
def main():
    ...
    def make_symbol_stim(name):
        ...
        if name == 'square':
            return visual.Rect(win, width=0.20, height=0.20, fillColor=dark, lineColor=None)
        if name == 'circle':
    ...
    key_chars = ['\u2190', '\u2193', '\u2192']
    key_labels = [visual.TextStim(win, text=c, pos=(x, -0.27), height=0.055, color='white')
    ...
    def bandit_trial():
        ...
        for i, ci in enumerate(corner_imgs):
            ...
            if i < len(pics) and pics[i]:
                ci.image = pics[i]
                ci.size = fit_size(pics[i], 0.20)    # fit a 0.20 box, aspect preserved
                drawables.append(ci)
