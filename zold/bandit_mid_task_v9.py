...
hardware.keyboard, parallel). Run from the PsychoPy Coder or `python bandit_mid_task_v9.py`.
Version v9 (see TASK_VERSION), logged in every data row.
...
# ════════════════════════════════════════════════════════════════════════════
TASK_VERSION = 'v9'              # stamped into every data row for provenance
...
def main():
    ...
    def bandit_trial():
        ...
        tilts = [int(srand() * 11) - 5 for _ in range(4)]   # srandInt(-5,5); 4 draws kept so the cosmetic stream stays aligned across win/loss
        pics = draw_win(srand, 4) if is_win else draw_images(srand, images['loss'], 4)
        drawables = [fb_center]
        sad_stims = []
        if is_win:
            ...
            # Loss: one image to the right of the points; the text+gap+image group
            # is centered as a unit. boundingBox is px, divided to height units.
            gap = 0.13                                   # wider gap clears the text and shifts the image right
            ...
            if pics and pics[0]:
                ...
                total = tw + gap + iw
                fb_center.pos = (-total / 2.0 + tw / 2.0, 0)
                sad_stims = make_sad_face(win, (total / 2.0 - iw / 2.0, 0), 0.30, ori=minor_tilt)
