...
CFG = dict(
    ...
    # Bandit timing
    FEEDBACK_MS=1250,
    ISI_MIN_MS=300,           # post-feedback blank, jittered per trial (cosmetic stream)
    ISI_MAX_MS=500,
    ANIM_MS=400,
...
def main():
    ...
    def bandit_trial():
        ...
        tilts = [int(srand() * 27) - 13 for _ in range(4)]   # srandInt(-13,13); 4 draws kept so the cosmetic stream stays aligned across win/loss
        ...
        if is_win:
            ...
            if pics and pics[0]:
                ...
                ci.ori = tilts[0]
                ci.image = pics[0]
                ci.size = fit_size(pics[0], 0.40)        # 2x box, aspect preserved
                iw = ci.size[0]
                total = tw + gap + iw
                ...
                total = tw + gap + iw
                fb_center.pos = (-total / 2.0 + tw / 2.0, 0)
                sad_stims = make_sad_face(win, (total / 2.0 - iw / 2.0, 0), 0.44, ori=tilts[0])
        ...
        hold([], isi_ms)                             # jittered blank ISI (300-500 ms)
