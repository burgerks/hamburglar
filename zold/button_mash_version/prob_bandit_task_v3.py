...
3-arm probabilistic reversal-learning task. Three reward profiles, one per arm:
    profile A: 80% reward (+10), 20% loss (-10)   (EV +6, best)
    profile B: 30% reward (+10), 70% loss (-10)   (EV -4, worst)
    profile C: 50% reward (+10), 50% loss (-10)   (EV  0, middle)
Outcomes are binary (+10 / -10). Two reversals at fixed trials cyclically
rotate all three profiles in random directions, so which arm is best changes twice.

Per session a random seed is drawn and logged in every row. That seed reproduces
the outcome sequence, the reversal directions, the symbol-to-arm assignment, the
screen placement, and the win/loss image order. The reversal trials and the
...
Sixteen ratings are logged (one before the first trial, then roughly balanced
across the three contingency phases, with one immediately before each reversal),
plus one unlogged practice rating.
...
CFG = dict(
    ...
    PROFILE_C=[0.50, 0.50],   # middle (EV  0)
    # 1-indexed trials at which reversals apply, before outcome sampling.
    # With 200 trials, this yields phases 1-68, 69-129, and 130-200.
    REVERSAL_TRIALS=[69, 130],
    ...
    # Trials BEFORE which a logged craving rating appears. Fixed for every participant.
    # Schedule: 5 ratings in phase 1, 5 in phase 2, 6 in phase 3; includes one
    # rating before trial 1 and one immediately before each reversal.
    WANTING_BEFORE_TRIALS=[1, 15, 29, 43, 57, 69, 82, 95, 108, 121, 130, 144, 158, 172, 186, 200],
    # Timing (ms)
    FEEDBACK_MS=1000,         # win/loss picture duration (food-cue window, no response)
    CUE_GAP_MS=600,           # response-free fixation after the food cue, isolates the iEEG epoch
    ITI_MS=500,
...
class State:
    ...
    def __init__(self, info, seed):
        ...
        self.last_wanting_before_trial = ""
...
def get_phase_info(trial_num):
    """Return phase metadata for a 1-indexed real-task trial number."""
    r1, r2 = CFG["REVERSAL_TRIALS"]
    if trial_num < r1:
        return dict(phase_num=1, phase_label="acquisition",
                    reversal_count_so_far=0, trials_since_reversal="",
                    is_reversal_trial=0)
    if trial_num < r2:
        return dict(phase_num=2, phase_label="reversal_1",
                    reversal_count_so_far=1, trials_since_reversal=trial_num - r1,
                    is_reversal_trial=int(trial_num == r1))
    return dict(phase_num=3, phase_label="reversal_2",
                reversal_count_so_far=2, trials_since_reversal=trial_num - r2,
                is_reversal_trial=int(trial_num == r2))


def apply_reversal_if_needed(S, rng):
    """At each reversal trial, rotate all three profiles in a random direction."""
    trial_num = S.trial + 1
    if trial_num in CFG["REVERSAL_TRIALS"]:
        p = S.profiles
...
# ===================================================================
COLUMNS = [
    "participant_id", "food_version", "seed", "event_index", "event_type",
    "phase", "phase_num", "phase_label", "reversal_count_so_far",
    "trials_since_reversal", "is_reversal_trial",
    ...
    "optimal_position", "is_optimal", "current_best_option", "current_best_prob",
    "current_middle_option", "current_middle_prob", "current_worst_option",
    "current_worst_prob", "chosen_option_rank", "optimal_points", "regret",
    "cumulative_score",
    ...
    # wanting-specific
    "wanting_index", "wanting_before_trial", "phase_at_wanting",
    "trials_since_last_wanting_rating", "craving_start", "craving_rating",
...
def send_trigger(code):
    """Send an event marker to the iEEG acquisition system at a screen flip.
    STUB: wire this to your recording rig. Common options:
      parallel port:  from psychopy import parallel
                      PORT = parallel.ParallelPort(address=0x0378)  # set once at startup
                      PORT.setData(int(code))                       # here, then setData(0) shortly after
      LSL:            push a sample on a pylsl StreamOutlet([int(code)])
      photodiode:     flash a small screen patch and detect it on a recording channel
    Use win.callOnFlip(send_trigger, code) so the marker fires exactly at the flip.
    Suggested codes: 1 = food-cue win, 2 = no-win, and add 10 = choice onset,
    20 = rating onset at those flips the same way."""
    pass


def run_cue_gap(win, kb):
    """Response-free fixation shown after the food cue, so the cue-evoked neural
    response sits in a clean window before the next trial starts."""
    txt(win, "+", height=0.06, color=COL["dim"]).draw()
    win.flip()
    wait_with_quit(win, kb, CFG["CUE_GAP_MS"] / 1000.0)
...
def show_feedback(win, kb, S, ch, rng, img_cache, win_next, loss_next,
    ...
    pts.draw()
    if not is_practice:
        win.callOnFlip(send_trigger, 1 if outcome == "reward" else 2)  # food-cue marker at onset
    fb_onset = win.flip()
...
def _log_choice_row(S, ch, outcome, points, cf_out, cf_pts, arm, fb_onset, img_path):
    ...
    trial_num = S.trial + 1
    phase_info = get_phase_info(trial_num)
    choice_pos = ch["pos"] + 1
    p_rewards = [p[0] for p in S.profiles]
    ranked_arms = sorted(range(CFG["N_ARMS"]), key=lambda a: p_rewards[a], reverse=True)
    opt_arm = ranked_arms[0]
    middle_arm = ranked_arms[1]
    worst_arm = ranked_arms[2]
    optimal_pos = S.slot_order.index(opt_arm) + 1
    optimal_points = cf_pts[opt_arm]

    def arm_position_label(a):
        return f"pos{S.slot_order.index(a) + 1}"
    ...
    S.rows.append(dict(
        ...
        **phase_info,
        ...
        trial=trial_num, swap_count=S.swap_count,
        ...
        current_best_option=arm_position_label(opt_arm),
        current_best_prob=round(p_rewards[opt_arm], 4),
        current_middle_option=arm_position_label(middle_arm),
        current_middle_prob=round(p_rewards[middle_arm], 4),
        current_worst_option=arm_position_label(worst_arm),
        current_worst_prob=round(p_rewards[worst_arm], 4),
...
def run_wanting(win, kb, S, is_practice, wanting_index="", before_trial=""):
    ...
    if not is_practice:
        phase_info = get_phase_info(before_trial)
        trials_since_last = (before_trial - S.last_wanting_before_trial
                             if S.last_wanting_before_trial != "" else "")
        S.last_craving = value
        S.last_wanting_before_trial = before_trial
        S.event_index += 1
        S.rows.append(dict(
            participant_id=S.pid, food_version=S.version, seed=S.seed,
            ...
            **phase_info,
            ...
            phase_at_wanting=phase_info["phase_label"],
            trials_since_last_wanting_rating=trials_since_last,
