...
stimuli/neutral cues/   neutral object images (neutral bonus cue)
stimuli/loss/           loss feedback images
```

`stimuli/symbols/{heart,circle,triangle}.png` are included. Empty folders fall
back to a placeholder (a labelled box for cues, a drawn sad face for losses) so
the task still runs for piloting.
